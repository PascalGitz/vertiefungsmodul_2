...
Analysen bewegen sich im ebenen Spannungszustand und beziehen sich auf
Stabtragwerke.
...
damit sowohl die erreichten Ergebnisse zusammenfasst als auch zukünftige
Forschungsperspektiven aufzeigt.
...
wurde das nicht-lineare Biegeverhalten verschiedener Versuche mittels
...
Stabstatikmodell basierend auf dem angewendeten Berechnungsansatz
erstellt. Die Abbildung~\ref{fig-modell_drehfeder} zeigt die
Modellierung eines einfachen Balkens, bei dem biege- und schubsteife
Stäbe mit Federn gekoppelt sind. In den folgenden Kapiteln wird diese
Modellierung als Federmodell bezeichnet.
...
gekoppelt mit Drehfedern, kurz Federmodell}

\end{figure}%

In dieser Modellierung erfolgen sämtliche Verformungsanteile des Systems
aus den Federverbindungen, welche in den FEM-Softwares als Stabengelenk
...
Steifigkeiten zuzuordnen. In den betrachteten 3D-Statiksoftwares können
den Stabendgelenken Verläufe mittels eines Diagrammeditors eingegeben
...
Momenten-Krümmungs-Beziehung des Querschnitts hergeleitet. Durch die
Einteilung in endlich lange Stäbe entspricht dies einer Diskretisierung
der Integration der Krümmungen. Schubverformungen lassen sich
beispielsweise durch die Bestimmung einer Kraft-Verformungs-Beziehung in
Wirkungsrichtung der Querkraft ermitteln. Es lässt sich vorwegnehmen,
dass die Wahl der Federcharakteristik die Krux des Systems darstellt.
...
Abbildung~\ref{fig-qs-kragarm} dargestellt.
...
den Zustandslinien der Schnittgrößen, die in
...
vorausgesetzt wird, entspricht der Faktor der Erhöhung des Verformung
dem Quotient der beiden Laststufen.
...
$$
\begin{aligned}
\chi_{1} &= \frac{ \varepsilon_{pu} - \frac{ \sigma_{P_{\infty}} }{ E_{p} } }{ d_{1} - x_{1} } \; 
\end{aligned}
$$

$$
\begin{aligned}
\chi_{1} &= 0.0154\ \frac{1}{\mathrm{m}} \;
\end{aligned}
$$
...
\(P_{max}\) der negative Biegewiderstand in etwa erreicht wurde. Dies
bedeutet, dass sich bei einer Steigerung der Last rechnerisch ein

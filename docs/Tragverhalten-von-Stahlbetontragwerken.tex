...
Seit der Revision der Schweizer Betonbau-Norm SIA 162 im Jahre 1994 ist
die Gebrauchstauglichkeit, insbesondere die Verformungen von Tragwerken,
zu einer zentralen Bemessungsgrösse geworden. Diese Änderung reflektiert
ein gesteigertes Bewusstsein für die Notwendigkeit, die langfristige
Leistungsfähigkeit und Sicherheit von Stahlbetontragwerken zu
gewährleisten. Trotz dieser elementaren Rolle im Bemessungsablauf werden
Verformungen im Stahlbeton in der heutigen Baupraxis häufig nur mit
groben Abschätzungen bewertet. Solche Ansätze können zu ungenauen
Prognosen führen, die sowohl die Sicherheit als auch die
Wirtschaftlichkeit von Bauprojekten beeinträchtigen können.
...
Masterthesis, ist es daher, mechanische Modelle zur Bestimmung von
Verformungen umfassend zu verstehen und vor allem praxisnah anzuwenden.
In der Vorarbeit {[}\citeproc{ref-gitz_ansatze_2024}{1}{]} wurden die
mechanischen Modelle detailliert beschrieben und anhand von
experimentellen Versuchen verifiziert, wobei mehrheitlich auf den
Einsatz kommerzieller FEM-Tools verzichtet wurde. Der Fokus dieser
Arbeit liegt nun auf der Integration von FEM-Tools mit den theoretischen
Grundlagen, um präzisere und praktisch anwendbare Lösungen zu
entwickeln. Verwendet wird hierbei die 3D Statiksoftware AxisVM-X7 und
RFEM-6, sowie die 2D Detailstatiksoftware IDEAStatiCa. Sämtliche
Analysen beziehen sich auf Stabtragwerke.
...
bereits gut verstandenen Versuchen aus der Vorarbeit verifiziert, um die
Zuverlässigkeit und Genauigkeit des Modells zu bestätigen. Abschliessend
wird die Anwendung an einem vorgespannten Träger demonstriert, um die
praktische Relevanz und Anwendbarkeit des entwickelten Modells zu
zeigen. Die Arbeit wird mit einem Fazit abgeschlossen, das eine
differenzierte Betrachtung zwischen Rückblick und Ausblick bietet und
damit sowohl die erreichten Ergebnisse zusammenfasst als auch
Erweiterungen aufzeigt.
...
im Bauingenieurwesen gelegt werden.

\bookmarksetup{startatroot}

\chapter{Modellvorstellung}\label{modellvorstellung}

Als einleitendes Kapitel wird hier die Modellvorstellung umschrieben und
deren Grundgedanke beleuchtet. Auf die Implementierung in einer
FEM-Software wird kurz eingegangen. Ein Einführungsbeispiel zeigt die
konzeptionelle Anwendung. Sowie werden bereits erste Vor- und Nachteile
der Modellbildung aufgezeigt.
...
gekoppelt mit Federn}

\end{figure}%

In dieser Modellierung erfolgen sämtliche Verformungsanteile des Systems
aus den Federverbindungen, welche in den FEM-Softwares als Stabendgelenk
modellierbar sind. Mit der Wahl der entsprechenden Federcharakteristiken
können passende Resultate erzielt werden. Dieser Ansatz ermöglicht es,
der Anzahl an Freiheitsgraden in den Stabendgelenken entsprechend
Steifigkeiten zuzuordnen. Den Rotationsfreiheitsgraden sind
Momenten-Verdrehungs-Beziehungen zu hinterlegen. Den
Translationsfreiheitsgraden sind Kraft-Verformungs-Beziehungen zu
hinterlegen. In den betrachteten 3D-Statiksoftwares können den
Stabendgelenken Verläufe mittels eines Diagrammeditors eingegeben
werden, welche beliebige nicht-lineare Beziehungen abbilden, sofern
diese numerisch stabil sind. Grundsätzlich wird die
...
Berechnung. Ein konzeptioneller Nachteil der Modellbildung ist die
Vielzahl an Diskretisierungen. Dies gilt allgemein für numerische
Lösungen. In diesem Modell fliesst die Wahl der Elementlänge mit ein.
Die Inkrementgrösse zur Bestimmung der Momenten-Krümmungs-Beziehung,
sofern diese numerisch bestimmt wurde, hat einen Einfluss. Zudem gilt es
bei einer nicht-lienaren Berechnung eine Inkrementgrösse zur Steigerung
der Last im Lösungsalgorithmus zu wählen. Diese Parameter haben einen
...
numerisch mit der Stabstatik-Software ermittelt.

Als Beispiel wurde ein Kragarm gewählt, der mit einer Drehfeder versehen
ist, welche eine nicht-lineare Drehfedercharakteristik aufweist. Es
werden zwei Laststufen betrachtet, die so gewählt sind, dass das
nicht-lineare Verhalten der Drehfeder deutlich wird. Das statische
System ist in Abbildung~\ref{fig-kragarm-feder} dargestellt.
...
In der ersten Laststufe wird eine geringe Last aufgebracht, um das
lineare Verhalten des Systems zu untersuchen. In der zweiten Laststufe
wird die Last erhöht, sodass das nicht-lineare Verhalten der Drehfeder
zum Tragen kommt. Diese Vorgehensweise ermöglicht es, den Übergang von
linearem zu nicht-linearem Verhalten detailliert zu analysieren und die
Auswirkungen der nicht-linearen Federcharakteristik auf die Verformungen
des Kragarms zu verdeutlichen.
...
\caption{\label{fig-qs-kragarm}Fiktiver Rechteckquerschnitt des
Kragarms}
...
Diese Darstellung zeigt, wie sich die Schnittgrößen des Kragarms unter
...
Dabei entspricht die maximale Biegeverformung am Ende des Kragarms ohne
Drehfeder für die Laststufe \(2\):
...
der \(YY\)-Dimension mit der Federcharakteristik ergänzt. Die
...
Laststufe. Dies ist auf die geringe Neigung, bzw. auf die Grösse von
\(k_{\varphi_2}\) der Drehfedercharakteristik im oberen Lastniveau
zurückzuführen. Die umgerechnete Wegfedercharakteristik ist in
...
nicht-linearen Verlaufs der Wegfedercharakteristik zurückführen. Das
Beispiel zeigt die Anwendung eines nicht-linearen
Momenten-Verdrehungs-Verlaufs anschaulich auf. Zudem ist die Problematik
der Diskterisierung, angesprochen in
Kapitel~\ref{sec-modellvorstellung}, veranschaulicht.
...
$$
\begin{aligned}
V_{Rd} &= a_{sw} \cdot z \cdot \frac{ f_{su} }{ \tan \left( \theta_{c3} \right) } \; 
\end{aligned}
$$

$$
\begin{aligned}
V_{Rd} &= 319.9\ \mathrm{kN} \;
\end{aligned}
$$
...
$$
\begin{aligned}
\gamma_{E_{A3V2}} &= z \cdot \frac{ 1 }{ \tan \left( \theta_{c3} \right) } \cdot \frac{1} { l_{E} }  = 140\ \mathrm{mm} \cdot \frac{ 1 }{ \tan \left( 34\ \mathrm{°} \right) } \cdot \frac{1} { 10\ \mathrm{mm} } &= 21\  
\end{aligned}
$$
...
Der Krümmungsverlauf gibt Aufschluss über den Fliessbereich der
Bewehrung, bzw. über den Steifigkeitenverlauf entlang der Stabachse. Die
Schubfeder bringt relativ viel Noise in die Resultate.
...
\chapter{Modellanwendung an einem vorgespannten
Träger}\label{modellanwendung-an-einem-vorgespannten-truxe4ger}
...
$$
\begin{aligned}
h_{sup} &= 180.0\ \mathrm{mm} \; 
 &h_{inf} &= 140.0\ \mathrm{mm} \; 
 &h &= 800.0\ \mathrm{mm} \; 
\\[10pt]
...
auf den Versuchsstand, sowie das Verhalten unter Belastung. Die
Abbildung~\ref{fig-durchbiegung_laengs_t6} zeigt den Verformungsverlauf
für den gesamten Träger für die entsprechenden Laststufen.
...
geschah schlagartig, durch das Versagen der Vorspannlitzen links neben
dem Auflager \(B\).
...
$$
\begin{aligned}
q_{c} &= 7.3\ \frac{\mathrm{kN}}{\mathrm{m}} \;
\end{aligned}
$$
...
Versuchslast. Zum einen wird die Einzellast \(P_{1}\) und die
entsprechende Streckenlast \(q_{1}\) angesetzt. Dies entspricht in etwa
der Laststufe 8. Dies ist in der Abbildung~\ref{fig-durchbiegung_t6}
aufgezeigt. Dieser Lastfall wird in Laststufen unterteilt bei der
Berechnung und liefert so Resultate für sämtliche Laststufen unterhalb
der Laststufe 8.
...
Der zweite Lastfall beinhaltet die konstante Einzellast \(P_{2}\), sowie
die Streckenlast \(q_{1}+ q_{\Delta}\). In diesem Lastfall wird
lediglich die Streckenlast \(q_{\Delta}\) in Laststufen unterteilt.

$$
\begin{aligned}
P_{2} &= 475.0\ \mathrm{kN} \; 
...
Nach Eurocode 2 bzw. DIN 1045 zur nicht-linearen
Schnittgrössenermittlung. Direkte Anwendung der Würfelfestigkeit als
Bauteilfestigkeit.
...
Es wird vorausgesetzt, dass die Vorspannlitzen die Fliessgrenze
erreichen. Die schlaffe Bewehrung hat ebenfalls die Fliessgrenze
erreicht. Die Spannungsverteilung des Betons wird als Dreiecksverlauf
angenommen.
...
\caption{\label{fig-t6_qs_MR}Betondruckzone mit dreieckiger
Spannungsverteilung, Bewehrung erreicht Fliessgrenze, Vorspannung
erreicht die Fliessgrenze}
...
$$
\begin{aligned}
x_{1} &= \frac{ A_{s_{1}} \cdot f_{sy} + A_{p} \cdot \left( f_{py} - \sigma_{P_{\infty}} \right) + P_{\infty} }{ b_{inf} \cdot 0.5 \cdot f_{c} } \; 
\end{aligned}
$$
...
$$
\begin{aligned}
M_{R1} &= \left( A_{s_{1}} \cdot f_{sy} + A_{p} \cdot \left( f_{py} - \sigma_{P_{\infty}} \right) \right) \cdot \left( d_{1} - S_{z} \right) + 0.5 \cdot b_{inf} \cdot f_{c} \cdot x_{1} \cdot \left( S_{z} - \frac{ 1 }{ 3 } \cdot x_{1} \right) \; 
\end{aligned}
$$

$$
\begin{aligned}
M_{R1} &= 1156.3\ \mathrm{kN} \cdot \mathrm{m} \;
\end{aligned}
$$
...
Es wird der Biegewiderstand betrachtet, unter der Prämisse dass die
Spannungsverteilung des Betons in der Druckzone einem Dreiecksverlauf
gleicht, sowie das Vorspannkabel die Zugfestigkeit erreicht. Der
schlaffen Bewehrung wird das Fliessen vorausgesetzt. Dieser Zustand ist
als Grenzzustand zu betrachten. Sind die numerischen Ergebnisse,
dargestellt im Anhang, kleiner oder gleich diesem Widerstand, so ist das
...
Betondruckzone mit dreieckiger Spannungsverteilung, Vorspannkabel
erreicht Zugfestigkeit, schlaffe Bewehrung fliesst}
...
$$
\begin{aligned}
x_{2} &= \frac{ A_{s_{1}} \cdot f_{sy} + A_{p} \cdot \left( f_{pu} - \sigma_{P_{\infty}} \right) + P_{\infty} }{ b_{inf} \cdot 0.5 \cdot f_{c} } \; 
\end{aligned}
$$
...
des Untergurts. Dies wird der Einfachkeits halber vernachlässigt.

Der Biegewiderstand bestimmt sich somit zu:

$$
\begin{aligned}
M_{R2} &= \left( A_{s_{1}} \cdot f_{sy} + A_{p} \cdot \left( f_{pu} - \sigma_{P_{\infty}} \right) \right) \cdot \left( d_{1} - S_{z} \right) + 0.5 \cdot b_{inf} \cdot f_{c} \cdot x_{2} \cdot \left( S_{z} - \frac{ 1 }{ 3 } \cdot x_{2} \right) \; 
\end{aligned}
$$
...
$$
\begin{aligned}
A_{s_{3}} &= 8 \cdot \frac{ \left( 14 \cdot \mathrm{mm} \right) ^{ 2 } }{ 4 } \cdot \pi \; 
\end{aligned}
$$

$$
\begin{aligned}
A_{s_{3}} &= 1231.5\ \mathrm{mm}^{2} \;
\end{aligned}
$$
...
$$
\begin{aligned}
x_{3} &= \frac{ A_{s_{3}} \cdot f_{sy} + P_{\infty} }{ b_{sup} \cdot 0.5 \cdot f_{c} } \; 
\end{aligned}
$$
...
$$
\begin{aligned}
M_{R3} &= A_{s_{3}} \cdot f_{sy} \cdot \left( d_{3} - S_{z} \right) + 0.5 \cdot b_{sup} \cdot f_{c} \cdot x_{3} \cdot \left( S_{z} - \frac{ 1 }{ 3 } \cdot x_{3} \right) \; 
\end{aligned}
$$

$$
\begin{aligned}
M_{R3} &= 724.2\ \mathrm{kN} \cdot \mathrm{m} \;
\end{aligned}
$$
...
$$
\begin{aligned}
x_{4} &= \frac{ A_{s_{3}} \cdot f_{sy} + P_{\infty} }{ b_{sup} \cdot 0.5 \cdot f_{c} } \; 
\end{aligned}
$$
...
$$
\begin{aligned}
M_{R4} &= A_{s_{3}} \cdot f_{sy} \cdot \left( d_{3} - S_{z} \right) + 0.5 \cdot b_{sup} \cdot f_{c} \cdot x_{4} \cdot \left( S_{z} - \frac{ 1 }{ 3 } \cdot x_{4} \right) \; 
\end{aligned}
$$
...
der Abbildung~\ref{fig-p_w10_vergleich} dargestellt. Die Nachrechnung
ist in etwa deckungsgleich mit den Messwerten bis zum Erreichen der
Höchstlast. Das Plateau im Plot wird mit der Nachrechnung nicht
getroffen. Betrachtet man die Belastungsgeschichte des Trägers gemäss
dem Versuchsbeschrieb, so wurde die Einzellast beim Erreichen der
höchstlast konstant gehalten. Das numerische Modell zeigt, dass bei
\(P_{max}\) der negative Biegewiderstand in etwa erreicht wurde.
...
Auflager \(B\) unter der Belastung \(LS8\)}

\end{figure}%

Dies bedeutet, dass sich bei einer Steigerung der Last rechnerisch ein
plastisches Gelenk ausbildet, folglich sich ein Mechanismus einstellt
beim Kragarm. Es lässt sich Vermuten dass dieses Verhalten bei der
Messung aufgezeichnet wurde. Die Verformungsfigur bestätigt diese
Vermutung. Dazu gilt es zu erwähnen, dass dich Last im Versuch
keineswegs absolut konstant gehalten werden kann. Kleine Schwankungen
der Last bei einem enorm weichen Verhalten liefern möglicherweise die
gemessenen Verformungen. Mit den gewählten vereinfachten bilinearen
Spannungs-Dehnungs-Beziehungen des Betonstahls und des Spannstahls wird
jedoch eine solche Präzision nicht angestrebt. Folglich ist das Resultat
zufriedenstellend, dass die Verformungen bis zum Erreichen der
Höchstlast nachgerechnet werden konnten.
...
für die Laststufe 8 für den Träger T6}
...
Verdrehungsverlauf für die Laststufe 8 für den Träger T6}
...
für die Laststufe 12 für den Träger T6}
...
Verdrehungsverlauf für die Laststufe 12 für den Träger T6}
...
\section{IdeaStatica}\label{ideastatica}
...
\subsection{Ergebnisse}\label{ergebnisse-2}
...
Resultate etwas weicher da Scheibenelemente auch Verschiebungen lateral
zur Längsachse berücksichtigen.
